 ...
 * Uses CoinGecko OHLC Data (4-hour candles)
...
const CONFIG = {
    ...
    // OHLC days - determines candle granularity:
    // 1-2 days = 30min candles, 3-30 days = 4hr candles, 31+ days = 4day candles
    ohlcDays: 7, // 7 days = 4-hour candles
    ...
    // Risk management
    takeProfitPercent: 2.0, // 2% TP for 4hr candles
...
// ============================================
// API FUNCTIONS - COINGECKO WITH CORS PROXY FALLBACK
// ============================================
...
async function fetchWithProxy(url) {
    ...
    try {
        ...
        console.error('CORS proxy also failed:', e.message);
    }
...
async function fetchCoinGeckoData() {
    try {
        // Fetch current price
        console.log('Fetching NEAR data from CoinGecko...');
        const priceUrl = `${CONFIG.geckoApi}/simple/price?ids=${CONFIG.geckoId}&vs_currencies=usd&include_24hr_change=true`;
        const priceData = await fetchWithProxy(priceUrl);

        if (!priceData || !priceData[CONFIG.geckoId]) throw new Error('Price fetch failed');

        const currentPrice = priceData[CONFIG.geckoId].usd;
        const priceChange = priceData[CONFIG.geckoId].usd_24h_change || 0;

        console.log(`Current NEAR price: $${currentPrice}`);

        // Fetch OHLC data
        const ohlcUrl = `${CONFIG.geckoApi}/coins/${CONFIG.geckoId}/ohlc?vs_currency=usd&days=${CONFIG.ohlcDays}`;
        const ohlcData = await fetchWithProxy(ohlcUrl);

        if (!ohlcData || !Array.isArray(ohlcData)) throw new Error('OHLC fetch failed');

        console.log(`Fetched ${ohlcData.length} candles (4-hour timeframe)`);
...
function updateLatestSignal() {
    ...
    const hoursAgo = Math.floor(timeSince / 60);
    const timeText = hoursAgo > 0 ? `${hoursAgo}h ${timeSince % 60}m ago` : `${timeSince}m ago`;
...
// ============================================
async function init() {
    console.log('Initializing NEAR Trading Dashboard...');
    console.log('Using CoinGecko OHLC data (4-hour candles)');
    ...
    if (!data || !data.candles || data.candles.length === 0) {
        document.getElementById('loadingOverlay').innerHTML = `
            <div class="no-signal">
                ...
                <div class="no-signal-sub">Please refresh the page</div>
            </div>
        `;
        return;
    }
    ...
    console.log('Refresh the page to update data');
}
